...
export default function initRoute(
  ...
  router.delete("/account/api/oauth/sessions/kill/:token", async (c) => {
    ...
    if (!token)
      return c.json(Errors.basic.badRequest.withMessage("the param 'token' is missing."), 400);

    const authHeader: string = c.req.header("Authorization")?.split("bearer ")[1] as string;
    const accessToken = authHeader.replace("eg1~", "");

    if (!accessToken) return c.json(Errors.authentication.invalidToken, 400);

    const decodedToken: JwtPayload = jwt.verify(
      accessToken,
      process.env.CLIENT_SECRET || "default-secret",
    ) as JwtPayload;

    if (typeof decodedToken === "string" || !decodedToken)
      return c.json(Errors.authentication.invalidToken, 400);
    ...
    if (!user || !account) return c.json(Errors.account.accountNotFound, 400);
  ...
  router.post("/account/api/oauth/token", async (c) => {
    ...
    if (!tokenHeader) return c.json(Errors.authentication.invalidHeader, 400);

    const token = tokenHeader.split(" ");

    if (token.length !== 2 || !Encoding.isValidBase64(token[1]))
      return c.json(
        Errors.authentication.oauth.invalidClient.withMessage("Base64 is not valid."),
        400,
      );
    ...
    try {
      ...
      return c.json(Errors.authentication.oauth.invalidBody, 400);
    }
    ...
    if (!clientId) return c.json(Errors.authentication.oauth.invalidClient, 400);
    ...
    switch (grant_type) {
      case "password":
        if (!password || !username)
          return c.json(
            Errors.basic.badRequest.withMessage("username or password is missing."),
            400,
          );

        user = await Users.findOne({ email: username as string });

        if (!user) return c.json(Errors.authentication.oauth.invalidAccountCredentials, 404);

        if (user.banned) return c.json(Errors.account.inactiveAccount, 400);

        if (await Hashing.verifyPassword(password as string, user.password))
          username = user.username;
        else return c.json(Errors.authentication.oauth.invalidAccountCredentials, 404);
        break;
      ...
      case "exchange_code":
        ...
        if (!exchange_code) return c.json(Errors.basic.badRequest, 400);

        const userToken: JwtPayload | null = verifyTokenAPI(exchange_code as string) as JwtPayload;

        if (!userToken) return c.json(Errors.authentication.oauth.invalidExchange, 400);

        user = await Users.findOne({ password: userToken.accountId });

        if (!user) return c.json(Errors.account.accountNotFound, 404);

        break;

      default:
        return c.json(Errors.authentication.oauth.unsupportedGrant, 400);
    }
    ...
    if (!account) return c.json(Errors.authentication.oauth.invalidAccountCredentials, 400);
  ...
  router.get("/account/api/oauth/verify", async (c) => {
    ...
    if (!user || !account) return c.json(Errors.account.accountNotFound, 404);
