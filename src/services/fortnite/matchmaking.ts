...
export default function initRoute(
  ...
  router.get(
    ...
    async (c) => {
      ...
      if (!accessToken || !account || !user) return c.json(Errors.account.accountNotFound, 404);

      if (accountId !== user.accountId) return c.json(Errors.authentication.notYourAccount, 400);
      ...
      if (bucketIds!.length < 4 || bucketIds!.length !== 4 || typeof bucketId !== "string")
        return c.json(Errors.matchmaking.invalidBucketId);

      if (!bucketIds![2] || !bucketIds) return c.json(Errors.matchmaking.invalidBucketId, 400);

      if (!userAgent) return c.json(Errors.internal.invalidUserAgent, 400);
      ...
      return c.json({
        serviceUrl: `ws://26.7.188.28:${env.MATCHMAKER_PORT}`,
        ...
        payload: JSON.stringify({
          ...
          attributes: {
            ...
            "player.privateMMS": customKey ? false : false,
  ...
  router.get("/fortnite/api/matchmaking/session/:sessionId", verify, async (c) => {
    ...
    if (!currentBuildUniqueId) return c.json(Errors.matchmaking.invalidBucketId, 400);
    if (!region || !playlist) return c.json(Errors.matchmaking.missingCookie, 400);
    ...
    return c.json({
      ...
      attributes: {
        ...
        HOTFIXVERSION_i: 0,
        PLAYLISTNAME_s: "ShowdownAlt_Solo",
