...
try {
  ...
  const rest = new REST({ version: "10" }).setToken(process.env.BOT_TOKEN || "default-bot-token");
  ...
  try {
    ...
    const endpoint = Routes.applicationGuildCommands(currentUser.id, "1479393226673619126"); // put your server id there
