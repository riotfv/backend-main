...
export default class RegisterCommand extends BaseCommand {
  ...
  async execute(interaction: CommandInteraction<CacheType>) {
    ...
    if (interaction.channelId !== (process.env.RegisterChannelID || "default-channel-id")) {
      const embed = new EmbedBuilder()
        .setTitle("Invalid Channel")
        .setDescription(`This command can only be used in <#${process.env.RegisterChannelID || "default-channel-id"}>`)
        .setColor("#F01414")
        .setTimestamp();
