...
export default async function SetMtxPlatform(
  ...
  if (!profile) return c.json(Errors.mcp.profileNotFound, 404);
  ...
  try {
    ...
    return c.json(Errors.proxy.invalidBody, 400);
  }
  ...
  if (!newPlatform)
    return c.json(Errors.basic.badRequest.withMessage("the param 'newPlatform' is missing."), 400);
