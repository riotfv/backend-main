...
export default async function EmptyGift(
  ...
  if (!account) return c.json(Errors.account.accountNotFound, 404);
  ...
  if (!athena || !common_core) return c.json(Errors.mcp.profileNotFound, 404);

  const season = ParseUserAgent(c.req.header("User-Agent"));

  if (!season) return c.json(Errors.internal.invalidUserAgent, 400);
  ...
  if (!receiverUser) return c.json(Errors.account.accountNotFound, 404);
  const receiverAccountId = senderAccountId;
  let receiverProfile = await Accounts.findOne({
    accountId: receiverAccountId,
  });
  if (!receiverProfile) return c.json(Errors.account.accountNotFound, 404);
