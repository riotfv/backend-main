...
app.use(async (ctx, next) => {
  if (env.ENABLE_LOGS) {
    if (ctx.req.path === "/images/icons/gear.png" || ctx.req.path === "/favicon.ico") await next();
      ...
      await next();
      logging.info(
        `${ctx.req.path} | (${getMethodColor(ctx.req.method)(ctx.req.method)}) | Status ${getStatusCodeColor(ctx.res.status)(ctx.res.status)}`,
        "backend",
      );
    }
  } else await next();
...
Promise.all([DatabaseHandler.connect(), RouteHandler.initializeRoutes(app)])
  .then(async () => {
    app.use(async (c, next) => {
      ...
      if (!missingUrl.has(fullUrl)) {
        return c.json(
          {
            errorCode: "errors.com.errors.common.not_found",
    ...
    logging.startup(`Backend listening on port ${env.PORT}`);
  })
  .catch((error) => {
    logging.error(`Error initializing Backend: ${error.message}`);
    process.exit(1);
  });